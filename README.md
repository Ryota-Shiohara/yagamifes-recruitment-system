...
`/` で応募者・面接官・局責任者のいずれかを選択します。選択後は、通常のリンクとURLを役割別の領域へ分けています。

- 応募者: `/applicant` 配下。応募情報の登録のみ。
- 面接官: `/interviewer` 配下。面接官の選択と評価入力。
- 局責任者: `/manager` 配下。応募者管理、面接枠、採用基準、ランキング。

ログイン機能は実装していないため、役割選択は画面導線を分けるためのものです。選択中の役割と異なる領域へアクセスした場合は、その役割のホームへ戻ります。
